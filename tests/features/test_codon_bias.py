...
class TestCodonBiasWindowing:
    ...
    def test_baseline_window_matches_direct_slicing_calls(self):
        ...
        assert "CB_CAI_GEOMEAN_3" in result.columns
        assert "CB_CAI_GEOMEAN_6" in result.columns
        assert abs(result["CB_CAI_GEOMEAN_0"].iloc[0] - expected_0) < 1e-6
        assert abs(result["CB_CAI_GEOMEAN_3"].iloc[0] - expected_3) < 1e-6
        assert abs(result["CB_CAI_GEOMEAN_6"].iloc[0] - expected_6) < 1e-6
    ...
    def test_rolling_matches_baseline_for_models_with_weights(self):
        ...
        assert abs(result["CB_CAI_GEOMEAN_0"].iloc[0] - expected_0) < 1e-3
        assert abs(result["CB_CAI_GEOMEAN_3"].iloc[0] - expected_3) < 1e-3
        assert abs(result["CB_CAI_GEOMEAN_6"].iloc[0] - expected_6) < 1e-3
...
class TestCodonBiasMultipleModels:
    ...
    def test_multiple_models_in_single_feature(self):
        ...
        assert "CB_CAI_GEOMEAN_6" in result.columns
        assert "CB_ENC_6" in result.columns
        assert "CB_FOP_MEAN_6" in result.columns
    ...
    def test_window_naming_uses_absolute_start_index(self):
        ...
        assert "CB_CAI_GEOMEAN_3" in result.columns
        assert "CB_CAI_GEOMEAN_6" in result.columns
        assert "CB_CAI_GEOMEAN_9" in result.columns
        assert "CB_CAI_GEOMEAN_12" in result.columns
...
class TestCodonBiasEdgeCases:
    ...
    def test_single_window_computation(self):
        ...
        feature_cols = [c for c in result.columns if c.startswith("CB_CAI_GEOMEAN_")]
